(function ($) {
  ...
  var dtc_url = "https://dtcts.piscifun.com/"
  $('.wt-code-item-video').click(function () {
      var videoCode = $(this).attr('video-code');
      if ($('.video-play-box').length == 0) {
        var html = '<div class="wt-code-ceng"></div><div class="video-play-box"><div class="p-r">';
        html += '<svg class="video-play-box-close" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg" width="62" height="62"><path d="M872.802928 755.99406 872.864326 755.99406 872.864326 755.624646Z" fill="#FFFFFF" p-id="2775"></path><path d="M927.846568 511.997953c0-229.315756-186.567139-415.839917-415.838893-415.839917-229.329059 0-415.85322 186.524161-415.85322 415.839917 0 229.300406 186.524161 415.84094 415.85322 415.84094C741.278405 927.838893 927.846568 741.29836 927.846568 511.997953M512.007675 868.171955c-196.375529 0-356.172979-159.827125-356.172979-356.174002 0-196.374506 159.797449-356.157629 356.172979-356.157629 196.34483 0 356.144326 159.783123 356.144326 356.157629C868.152001 708.34483 708.352505 868.171955 512.007675 868.171955" fill="#FFFFFF" p-id="2776"></path><path d="M682.378947 642.227993 553.797453 513.264806 682.261267 386.229528c11.661597-11.514241 11.749602-30.332842 0.234337-41.995463-11.514241-11.676947-30.362518-11.765975-42.026162-0.222057L511.888971 471.195665 385.223107 344.130711c-11.602246-11.603269-30.393217-11.661597-42.025139-0.059352-11.603269 11.618619-11.603269 30.407544-0.059352 42.011836l126.518508 126.887922L342.137823 639.104863c-11.662621 11.543917-11.780301 30.305213-0.23536 41.96988 5.830799 5.89015 13.429871 8.833179 21.086248 8.833179 7.53972 0 15.136745-2.8847 20.910239-8.569166l127.695311-126.311801L640.293433 684.195827c5.802146 5.8001 13.428847 8.717546 21.056572 8.717546 7.599072 0 15.165398-2.917446 20.968567-8.659217C693.922864 672.681586 693.950494 653.889591 682.378947 642.227993" fill="#FFFFFF" p-id="2777"></path></svg>';
        html += '<div class="main-search__results-spinner" style="padding-top: 25%; margin: 0;"><div class="loading-spinner"></div></div> Loading...<iframe id="ytplayer" type="text/html" src="https://www.youtube.com/embed/' + videoCode + '" allow="autoplay" allowfullscreen="1" style="width: 100%; height: 100%;"></iframe>';
        html += '</div>';
        html += '</div>';
  ...
  /** run section **/
  var sectionList = {};
  var runSectionList = function (obj) {
    console.log(' - sectionList', sectionList);
    for (var productId in sectionList) {
      var html = '<img src="' + sectionList[productId] + '" />';
      
      $(obj).find('.product-block[data-product-id="' + productId + '"]').find('.product-link').eq(0).find('.image-label-wrap').hide();
      $(obj).find('.product-block[data-product-id="' + productId + '"]').find('.product-link').eq(0).append(html);
    }
  }
  var runSection = function () {
      var url = 'https://assets-us.welltuo.com/json/c-10-piscifun/section.json?' + (Math.random() + '').substr(2);
      if (window.location.pathname == '/pages/home-page') {
        url = 'https://assets-us.welltuo.com/json/c-10-piscifun_home/section.json?' + (Math.random() + '').substr(2);
      }
      $.ajax({
        url: url,
        success: function (result) {
          ...
          if (document.getElementById('content')) {
            var list = document.getElementById('content').getElementsByClassName('shopify-section');
            if (list[result.number]) {
              $(list[result.number]).find('.image-page-button--previous').hide();
              $(list[result.number]).find('.image-page-button--next').hide();
              $(list[result.number]).find('h2 .has-paging__title').attr('href', result.uri);
              $(list[result.number]).find('.small-feature-link').attr('href', result.uri);
              var plist = $(list[result.number]).find('.owl-item');
              for (var idx = 0; idx < plist.length; idx++) {
                if (result.image_obj[idx]) {
                  var productId = $(plist[idx]).find('div').attr('data-product-id');
                  sectionList[productId] = result.image_obj[idx];
                }
              }
    
              runSectionList(list[result.number]);
            }
          }
        }
      })
  }
  if (window.location.pathname == '/' || window.location.pathname == '/pages/home-page') {
    runSection();
  }
  /** end section **/

  // --- shopify-countdown --- // 
  var runCountdown = function (div, endTimeStr) {
    var nowTime = new Date();
    var endTime = new Date(endTimeStr);
    var lefttime = endTime.getTime() - nowTime.getTime();
    var html = '';
    if (lefttime < 0) {
      html += '<span class="countdown-time-number">0</span>';
      html += '<span class="countdown-time-number">0</span>';
      html += ':';
      html += '<span class="countdown-time-number">0</span>';
      html += '<span class="countdown-time-number">0</span>';
      ...
      if (leftd < 10) {
        html += '<span class="countdown-time-number">0</span>';
        html += '<span class="countdown-time-number">' + leftd + '</span>';
      } else {
        html += '<span class="countdown-time-number">' + ((leftd + '')[0]) + '</span>';
        html += '<span class="countdown-time-number">' + ((leftd + '')[1]) + '</span>';
      }
      html += ':';
      if (lefth < 10) {
        html += '<span class="countdown-time-number">0</span>';
        html += '<span class="countdown-time-number">' + lefth + '</span>';
      } else {
        html += '<span class="countdown-time-number">' + ((lefth + '')[0]) + '</span>';
        html += '<span class="countdown-time-number">' + ((lefth + '')[1]) + '</span>';
      }
      html += ':';
      if (leftm < 10) {
        html += '<span class="countdown-time-number">0</span>';
        html += '<span class="countdown-time-number">' + leftm + '</span>';
      } else {
        html += '<span class="countdown-time-number">' + ((leftm + '')[0]) + '</span>';
        html += '<span class="countdown-time-number">' + ((leftm + '')[1]) + '</span>';
      }
      html += ':';
      if (lefts < 10) {
        html += '<span class="countdown-time-number">0</span>';
        html += '<span class="countdown-time-number">' + lefts + '</span>';
      } else {
        html += '<span class="countdown-time-number">' + ((lefts + '')[0]) + '</span>';
        html += '<span class="countdown-time-number">' + ((lefts + '')[1]) + '</span>';
      }
    }
    div.html(html);
    setTimeout(function () {
      runCountdown(div, endTimeStr);
    }, 1000);
  }
  
  var runCountdownContainer = function () {
    $('.countdown-container').each(function () {
      if ($(this).find('.pagetitle').length > 0) {
        if (window.innerWidth > 758) {
          $(this).find('.pagetitle').css('text-align', 'left');
          $(this).find('.pagetitle').css('position', 'relative');
        } else {
          $(this).find('.pagetitle').css('position', 'relative');
          $(this).find('.collection-listing').css('margin-top', '80px');
        }
        $(this).find('.pagetitle').append('<div class="countdown-div">Ends in: <div class="countdown-time"></div></div>');
      } else {
        $(this).find('.hometitle').append('<div class="countdown-div">Ends in: <div class="countdown-time"></div></div>');
        if (window.innerWidth > 758) {
          $(this).find('.hometitle').css('text-align', 'left');
          $(this).find('.hometitle').css('position', 'relative');
        } else {
          $(this).find('.hometitle').css('position', 'relative');
          if (($(this).find(".flash-sale-mobile").length > 0 && $(this).find(".flash-sale-mobile .view-all").length == 0) || ($(this).find(".flash-sale-mobile").length == 0 && $(this).find(".view-all").length == 0)) {
              $(this).find(".countdown-div").css("top","40px");
          }
          $(this).find('.collection-listing').css('margin-top', '80px');
          
        }
        $(this).find('.product-block__title').addClass("two-lines");

      }
      
      runCountdown($(this).find('.countdown-time'), $(this).attr('end_time'));
        
      var labelList = $(this).find('.collection-listing .product-label-container-new .product-label-new--sale');
      for (var idx = 0; idx < labelList.length; idx++) {
        var spanstr = labelList.eq(idx).html();
        var sale_amount = labelList.eq(idx).find(".sale_amount").html()
        labelList.eq(idx).css('background', '#FFD03F');
        labelList.eq(idx).css('font-size', '12px');
        labelList.eq(idx).css('display', 'flex');
        labelList.eq(idx).css('margin-left', '4px');
        labelList.eq(idx).css('align-items', 'center');
        labelList.eq(idx).css('width', '61px');
        spanstr = '<img class="product-label-new-icon" src="https://cdn.shopify.com/s/files/1/1515/7758/files/flash-icon.png?v=1698371759" style="height: 16px;width:16px !important; vertical-align: middle;" /><span style="color:#000000;font-size:12px;padding:0 3px">-' + sale_amount+'</span>';
        labelList.eq(idx).html(spanstr);
      }

      if (window.location.pathname != '/pages/home-page') {
        $(this).find('.product-block__title').css('color', '#000');
        $(this).find('.product-block__title').css('font-weight', '500');
        $(this).find('.product-price').css('color', '#000');
      }
      
    });
    
    $('.countdown-span-time').each(function () {
      runCountdown($(this), $(this).attr('end_time'));
    });
  };

  var countdownListArr = {};
  var runHomeEndIn = function () {
      // var url = 'https://assets-us.welltuo.com/json/c-10-piscifun/end-in.json?' + (Math.random() + '').substr(2);
      // if (window.location.pathname == '/pages/home-page') {
      //   url = 'https://assets-us.welltuo.com/json/c-10-piscifun_home/end-in.json?' + (Math.random() + '').substr(2);
        // }
      var url = dtc_url+'api/activity/flashSale?shop_id=1&activity_site=us&current='+ (Math.random() + '').substr(2);
      $.ajax({
        url: url,
        timeout: 3000, // 设置超时时间为 3 秒
        success: function (result) {
          console.log(result);
          // result.end_in = "2023-12-15 12:00:00";
          // result.number = 1;
          if (result.end_in) {
            if (document.getElementById('content')) {
              var list = document.getElementById('content').getElementsByClassName('shopify-section');
              if (list[result.number]) {
                list[result.number].attributes.class.value = list[result.number].attributes.class.value + ' countdown-container';
                list[result.number].setAttribute('end_time', result.end_in);
                runCountdownContainer();
                $('.countdown-container').each(function () {
                  $(this).find('.product-list .block-inner').each(function (e) {
                    if (result.inv_obj[e]) {
                      var productId = $(this).parent().attr('data-product-id');
                      countdownListArr[productId] = result.inv_obj[e];
                      $(this).append('<div class="countdown-speed" style="width: 100%; margin-top: -25px; height: 15px; background: #FFF1C6; border-radius: 10px;"><div style="color: #000; font-size: 12px; line-height: 15px; white-space: pre; width: ' + result.inv_obj[e] + '; height: 15px; background: #FFD03F; border-radius: 10px; text-align: left; padding-left: 10px;">Only ' + result.inv_obj[e] + ' left</div></div>');
                    }
                  });

                  $(this).find('.flash-sale-mobile .product-list .block-inner').each(function (e) {
                    if (result.inv_obj[e]) {
                      var productId = $(this).parent().attr('data-product-id');
                      countdownListArr[productId] = result.inv_obj[e];
                ...
                console.log('countdownListArr', countdownListArr);
  ...
  var runProductDetailsEndIn = function () {
      ...
      $.ajax({
        ...
        success: function (result) {
          console.log(result);
          if (result.end_in && result.collection_id) {
            
            var collectionsAttr = $('.flash-div').attr('data-collections'); // 获取data-collections属性值
            if (collectionsAttr) {
                 var collectionsArray =collectionsAttr.split(',')
                console.log(collectionsArray)
                var collectionIdToCheck = result.collection_id.toString();
                var isCollectionIdExists = collectionsArray.includes(collectionIdToCheck); // 检查集合ID是否存在
          
                if (isCollectionIdExists) {
                  $('.countdown-span-time').attr('end_time', result.end_in);
                  $('.flash-div').show();
                  runCountdownContainer();
                } 
            }
  ...
  var runCountdownListArr = function () {
    $('.countdown-container').each(function () {
      $(this).find('.owl-item .block-inner').each(function (e) {
        if ($(this).find('.countdown-speed').length == 0) {
          var productId = $(this).parent().attr('data-product-id');
          if (countdownListArr[productId]) {
            $(this).append('<div class="countdown-speed" style="width: 100%; margin-top: -25px; height: 15px; background: #FFF1C6; border-radius: 10px;"><div style="color: #000; font-size: 12px; line-height: 15px; white-space: pre; width: ' + countdownListArr[productId] + '; height: 15px; background: #FFD03F; border-radius: 10px; text-align: left; padding-left: 10px;">' + countdownListArr[productId] + ' Sold</div></div>');
          }
        }
      });
    });
  ...
  */
  ...
  theme.addedToCartHandler = function (response) {
    ...
    //ech0
    if (typeof window.aa_freegift !== "undefined") {
      console.log('Update aa_freegift notice when ajax add to cart');
      window.aa_freegift.saveActivityProductAdded();
      var aaFreeGiftElementShow = $('body').attr('data-aafreegiftelementshow');
      $('body').attr('data-aafreegift-force-update-notice', 1);
      aaFreeGiftElementShow = parseInt(aaFreeGiftElementShow);
      window.aa_freegift.notice_render(aaFreeGiftElementShow, window.aa_freegift.getCart());
    }
    //ech0
  };
  ...
  theme.OptionManager = new function () {
    ...
    _.updatePreSales = function (variant, $container) {
      $container.find('.variant-visibility-area-pre-sales').each(function () {
        $(this).children(':not(script)').remove();
        $(".pre-order-btn").hide();
        $("#dynamic-payment-div").show();
        if (variant) {
          try {
            jsonData = JSON.parse($(this).children('script[data-variant-id="' + variant.id + '"]').html());
            if (jsonData.is_pre_sales == 1) {
              console.log("---inside---")
              $(this).append('<div>'+jsonData.remark+'</div>');
              $(".pre-order-btn").show();
              $("#dynamic-payment-div").hide();
            }else{
              console.log("---outside---")
              $(this).append($(this).children('script[data-variant-id=""]').html());
            }
    ...
    _.initProductOptions = function ($productForm) {
      ...
      $productForm.find(_.selectors.primaryVariantIdInput).each(function () {
        ...
        $(this).on('change.themeProductOptions firstrun.themeProductOptions', function (e) {
          ...
          _.updatePreSales(variant, $container)
      ...
      refreshCartDependentContent: function refreshCartDependentContent() {
        ...
        this.cartRefreshXhr = $.ajax({
          ...
          success: function (data) {
            ...
            //ech0
            if (typeof window.aa_freegift !== "undefined") {
              console.log('Reload aa_freegift when update cart - 1');
              var aaFreeGiftElementShow = $('body').attr('data-aafreegiftelementshow');
              var aaFreeGiftCart = window.aa_freegift.getCart();
              var aaFreeGiftProductList = window.aa_freegift.getProductList(aaFreeGiftCart);
              window.aa_freegift.checkGiftInCartData(aaFreeGiftCart);
              if (aaFreeGiftElementShow == '1') {
                
                console.log('Reload aa_freegift when update cart - 2');
                window.aa_freegift.cartRemoveGift(aaFreeGiftCart, aaFreeGiftProductList, false);
                window.aa_freegift.checkoutLastCheck(aaFreeGiftCart);
                window.aa_freegift.cartFreeGiftItems(aaFreeGiftProductList);
                window.aa_freegift.getProductCartSwiperInit();
                window.aa_freegift.notice_render(aaFreeGiftElementShow, aaFreeGiftCart);
                window.aa_freegift.modal_foot_button(aaFreeGiftCart);
                
              } else {
                console.log('Reload aa_freegift when update cart - 3');
                window.aa_freegift.cartRemoveGift(aaFreeGiftCart, aaFreeGiftProductList, true);
              }
              if (typeof window.aa_freegift.apiCollections_urls !== "undefined" && window.aa_freegift.apiCollections_urls != '') {
                $('.aa-freegift-cart .aa-head .aa-freegift-notice').attr('data-link', window.aa_freegift.apiCollections_urls);
              }
            }
            //ech0
  ...
  $(function ($) {
    ...
    $(document).on('load-product-form', '.product-form', function (e) {
      ...
      $(this).on('variantChanged', function (evt, variant, product) {
        // alert(variant.metafields.custom.pre_product)
  ...
  const storeFrontClient = ShopifyStorefrontAPIClient.createStorefrontApiClient({
    storeDomain: 'http://8c82e6-3.myshopify.com',
    apiVersion: '2023-10',
    publicAccessToken: '5a4a756ec583678b3728e5c8064263f6',
  });

  window.storeFrontClient = storeFrontClient

  $('body').on('click', '#pagefooter', function () {
    const shopQuery = `
      query shop {
        shop {
          name
    ...
    `;
    storeFrontClient.fetch(shopQuery);
  })
